...
(aka master) Nameserver when nameserver_ha is set to true.

Default: the current IP (at install)  

==== nameserver_ha
Set to true to configure Nameserver high-availability (master/slave).

Note: nameserver_ha requires at least 2 broker servers for high-availability.

Default: false

==== nameserver_members
An array of Nameserver IP addresses. Requires setting nameserver_ha to true.
The array should start with the Nameserver master IP address, followed by
Nameserver Slave IP address(es).

Default: undef

=== nameserver_master
Specifies whether the server is a Nameserver master or slave. Available options
are true for Nameserver master and false for Nameserver slave.

Note: Requires setting nameserver_ha to true.

...
class openshift_origin::plugins::frontend::apache {
  ...
  if 'broker' and 'load_balancer' in $::openshift_origin::roles {
    ...
    exec { 'ssl_conf':
      path    => ['/bin/', '/sbin/', '/usr/bin/', '/usr/sbin/'],
      command => "sed -ri \'s/Listen 443 https/Listen ${openshift_origin::broker_ip_addr}:443 https/\' /etc/httpd/conf.d/ssl.conf",
      unless  => "grep \"Listen ${openshift_origin::broker_ip_addr}:443 https\" /etc/httpd/conf.d/ssl.conf",
      require => Package['httpd'],
      notify  => Service['httpd'],
    }
  }
